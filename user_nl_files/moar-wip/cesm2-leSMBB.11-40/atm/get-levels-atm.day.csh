...
set doThis = 0
if ($doThis == 1) then
foreach v (`cat atm.day.h1`)
	nh $DOUT_S_ROOT/atm/proc/tseries/day_1/$CASE.cam.h1.$v.1850* | grep float | grep lev | grep -v ilev >> atm.day.h1.3D.list
	nh $DOUT_S_ROOT/atm/proc/tseries/day_1/$CASE.cam.h1.$v.1850* | grep float | grep ilev >> atm.day.h1.15L.list
	nh $DOUT_S_ROOT/atm/proc/tseries/day_1/$CASE.cam.h1.$v.1850* | grep float | grep -v lev >> atm.day.h1.2D.list
end

cat atm.day.h1.3D.list | cut -d" " -f2 | cut -d"(" -f1 > atm.day.h1.3D.txt
cat atm.day.h1.2D.list | cut -d" " -f2 | cut -d"(" -f1 > atm.day.h1.2D.txt
cat atm.day.h1.15L.list | cut -d" " -f2 | cut -d"(" -f1 > atm.day.h1.15L.txt
endif

rm atm.day.h6.3D.list atm.day.h6.15L.list atm.day.h6.2D.list 
foreach v (`cat atm.day.h6`)
