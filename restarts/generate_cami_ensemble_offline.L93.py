...
def get_rvals(date, ensemble_start,ensemble_end, model):
    ...
    rvals_file = os.path.join("/glade/campaign/cesm/development/espwg/CESM3-SMYLE/inputdata/","camic","pert_"+date+"{}-{}.txt".format(ensemble_start,ensemble_end))
    #rvals_file = os.path.join("/glade/campaign/cesm/development/espwg/CESM3-SMYLE/inputdata/","camic","pert_"+date+".{}-{}.txt",format(ensemble_start,ensemble_end))
...
def create_cam_ic_perturbed(original, ensemble_start,ensemble_end, date, baserundir, model, outroot="b.e21.SMYLE_IC.pert.ne30np4_L93_t233_wgx3.cam.i.", factor=0.15):
    ...
    if model == "cesm2smyle":
        #local_path = "/glade/derecho/scratch/islas/SMYLE_ERA5_L83pert/REORDERED/"
        #local_path = "/glade/campaign/cesm/development/espwg/SMYLE-ERA5-L83/initialconditions/SMYLE_ERA5_L83pert/REORDERED/"
        #local_path = "/glade/campaign/cgd/cas/islas/ERA5_IC/perts/MT/"
        local_path = "/glade/campaign/cesm/development/espwg/CESM3-SMYLE/inputdata/initial_conditions/"
    else:
        #local_path = "/glade/derecho/scratch/islas/SMYLE_ERA5_L83pert/REORDERED/"
        #local_path = "/glade/campaign/cesm/development/espwg/SMYLE-ERA5-L83/initialconditions/SMYLE_ERA5_L83pert/REORDERED/"
        #local_path = "/glade/campaign/cgd/cas/islas/ERA5_IC/perts/MT/"
    ...
    for i in range(ensemble_start,ensemble_end, 2):
        ...
        if model == "cesm2smyle":
            perturb_file = os.path.join("nanr-{}-01".format(month),
                                        "pert_{:04d}.nc".format(rvals[(i-1)//2]))
    ...
    pertroot = os.path.join("/glade/campaign/cesm/development/espwg/CESM3-SMYLE/inputdata/cesm3_init/","b.e30.SMYLE_IC.ne30np4_L93_t233_wgx3."+date[0:7]+".01","pert.01")
...
def create_perturbed_init_file(original, perturb_file, outfile, weight):
    ...
    if "BWHIST" in original:
        ...
        cmd1 = ncflint+" -O -C -v U,V,T,Q,PS -w {},1.0 {} {} {}".format(weight, perturb_file, original, outfile)
        ...
        cmd2 = ncks+" -A -v lon,lat,lev,ilev,hyai,hybi,hyam,hybm,PHIS,W,P_00,area {} {}".format(original, outfile)
        #cmd3 = ncrename+" -d ncol_d,ncol {} {}".format(original, outfile)
    run_cmd(cmd1, verbose=True)
    run_cmd(cmd2, verbose=True)
    #run_cmd(cmd3, verbose=True)
...
def _main_func(description):
    ...
    sdrestdir = os.path.join("/glade/campaign/cesm/development/espwg/CESM3-SMYLE/inputdata/cesm3_init","b.e30.SMYLE_IC.ne30np4_L93_t233_wgx3."+date[0:7]+".01","{}".format(date))
    user = os.getenv("USER")
    baserundir = os.path.join("/glade/derecho/scratch/{}/".format(user),"CESM3-SMYLE","b.e30_alpha09a.BHISTC_MTso_SMYLE.ne30_t233_wgx3."+date[0:7]+".001","run.{:03d}".format(ensemble_start))
    caminame = os.path.join(sdrestdir,"b.e30.SMYLE_IC.ne30np4_L93_t233_wgx3.{}.01.cam.i.{date}-00000.nc".format(date[:7],date=date))
    outroot = "b.e30.SMYLE_IC.pert.ne30np4_L93_t233_wgx3.cam.i."
