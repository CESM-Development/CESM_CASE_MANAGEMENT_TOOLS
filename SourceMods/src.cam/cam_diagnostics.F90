...
integer, parameter :: surf_092500 = 3
integer, parameter :: surf_085000 = 4
integer, parameter :: surf_070000 = 5
integer, parameter :: nsurf = 5
...
!==============================================================================
  ...
  subroutine diag_init_dry(pbuf2d)
    ...
    call addfld ('OMEGA050',   horiz_only,  'A', 'Pa/s',      'Vertical velocity at 050 mbar pressure surface')
    call addfld ('OMEGA010',   horiz_only,  'A', 'Pa/s',      'Vertical velocity at 010 mbar pressure surface')
    ...
    call addfld ('U050',       horiz_only,  'A', 'm/s','Zonal wind at 050 mbar pressure surface')
    ...
    call addfld ('V050',       horiz_only,  'A', 'm/s','Meridional wind at 050 mbar pressure surface')
    call addfld ('V010',       horiz_only,  'A', 'm/s','Meridional wind at 010 mbar pressure surface')
    ...
    if (history_vdiag) then
      ...
      call add_default ('OMEGA1000', 3, ' ')
      call add_default ('OMEGA700', 3, ' ')
      call add_default ('OMEGA500', 3, ' ')
      call add_default ('OMEGA200', 3, ' ')
      call add_default ('OMEGA100', 3, ' ')
      call add_default ('OMEGA050', 3, ' ')
      call add_default ('OMEGA010', 3, ' ')
    end if
...
!===============================================================================
  ...
  subroutine diag_phys_writeout_dry(state, pbuf, p_surf_t)
    ...
    if ( hist_fld_active('T950')       then
      call vertinterp(ncol, pcols, pver, state%pmid, 95000._r8, state%t, p_surf_t(:,surf_95000))
    end if
    if ( hist_fld_active('T900')       then
      call vertinterp(ncol, pcols, pver, state%pmid, 90000._r8, state%t, p_surf_t(:,surf_90000))
    end if
...
!!! at 1000 mb and 925 mb
    ...
    if (hist_fld_active('Q850')) then
      call outfld('Q850    ', p_surf_q2, pcols, lchnk )
    end if
    if (hist_fld_active('Q700')) then
      call outfld('Q700    ', p_surf_q2, pcols, lchnk )
    end if
    if (hist_fld_active('Q500')) then
      call outfld('Q500    ', p_surf_q2, pcols, lchnk )
    end if
    if (hist_fld_active('Q200')) then
      call outfld('Q200    ', p_surf_q2, pcols, lchnk )
    end if
    if (hist_fld_active('Q100')) then
      call outfld('Q100    ', p_surf_q2, pcols, lchnk )
    end if
    if (hist_fld_active('Q050')) then
      call outfld('Q050    ', p_surf_q2, pcols, lchnk )
    end if
    if (hist_fld_active('Q010')) then
      call outfld('Q010    ', p_surf_q2, pcols, lchnk )
    end if
