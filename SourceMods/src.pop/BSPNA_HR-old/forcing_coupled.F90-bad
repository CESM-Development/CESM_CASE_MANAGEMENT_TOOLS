...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_NAO_F          ! tavg id for NAO heat flux forcing
...
!-----------------------------------------------------------------------
!  variables for Lab Sea NAO heat flux forcing
!-----------------------------------------------------------------------
   real (r8), dimension(:,:,:), allocatable :: &
      NAO_F,      &  ! NAO heat flux forcing
      NAO_F_NOW      ! Current NAO_F
...
!-----------------------------------------------------------------------
!
!  variables associated with NAO forcing data read
!
!-----------------------------------------------------------------------

   type (datafile) :: &
      NAO_forcing_file  ! data file descriptor for Lab Sea NAO HFLX forcing

   type (io_field_desc) :: &
      NAO_F_data_in          ! io field descriptor for input HFLX data

   type (io_dim) :: &
      i_dim, j_dim ! dimension descriptors for horiz dims

   character (char_len) ::  &
      NAO_forcing_filename, &
      NAO_forcing_file_fmt
...
!-----------------------------------------------------------------------
!
!  allocate space for and read NAO frocing
!
!-----------------------------------------------------------------------

   NAO_forcing_filename = '/glade/work/whokim/cesm_forcing/NAO_forcing/WISHBONE/SHF_SPNA_DJFM_NAO_2X_forcing_ERA5_to_TL319_t13.nc'
   NAO_forcing_file_fmt = 'nc'

   if (my_task == master_task) then
      write(stdout,blank_fmt)
      write(stdout,1030) ' NAO_forcing_file      = ',  NAO_forcing_filename
      write(stdout,blank_fmt)

1030  format (a26,2x,a90)
   endif

   allocate(NAO_F(nx_block,ny_block,max_blocks_clinic),      &
            NAO_F_NOW(nx_block,ny_block,max_blocks_clinic))
!            ADJ_F(nx_block,ny_block,max_blocks_clinic),  &
!            ADJ_MASK(nx_block,ny_block,max_blocks_clinic))

   NAO_F = c0; NAO_F_NOW = c0
!   ADJ_F = c0; ADJ_MASK = c0

   NAO_forcing_file = construct_file(NAO_forcing_file_fmt,            &
                                   full_name=trim(NAO_forcing_filename), &
                                   recl_words=nx_global*ny_global)

   call data_set (NAO_forcing_file, 'open_read')

   i_dim = construct_io_dim('i',nx_global)
   j_dim = construct_io_dim('j',ny_global)

  NAO_F_data_in = construct_io_field('SHF_NAO',dim1=i_dim, dim2=j_dim,   &
                             field_loc = field_loc_center,              &
                             field_type = field_type_scalar,            &
                             d2d_array = NAO_F)

   call data_set (NAO_forcing_file, 'define', NAO_F_data_in)
   call data_set (NAO_forcing_file, 'read',   NAO_F_data_in)
   call destroy_io_field (NAO_F_data_in)

   call data_set (NAO_forcing_file, 'close')
   call destroy_file (NAO_forcing_file)
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_IFRAC,'IFRAC',2,                                &
                          ...
                          coordinates='TLONG TLAT time')
   call define_tavg_field(tavg_NAO_F,'NAO_F',2,                                &
                          long_name='NAO Heat Flux from Forcing',              &
                          units='watt/m^2', grid_loc='2110',                   &
                          coordinates='TLONG TLAT time')
...
!-----------------------------------------------------------------------
   ...
   real (r8) :: &
      h_imb,    &   ! area-averaged heat imbalance
!       h_adj,    &   ! heat flux for adjustment 
      wt_nao        ! nao ramping weight for this timestep
...
!-----------------------------------------------------------------------
   ...
   !*** experimental code: NAO forcing

   !*** No NAO forcing after May of 5th year 
   if ( elapsed_months_init_date < 112 ) then
     call nao_ramp_weights (wt_nao)
!     wt_nao = wt_nao*3

!####### DEBUG
     if (my_task == master_task) then
       write(stdout,blank_fmt)
       write(stdout,'(a,1x,i4.4,a,i2.2,a,i2.2,1x,a,F16.6)') &
         '(pop_set_coupled_forcing)',iyear,'-',imonth,'-',iday, 'wt_nao = ', wt_nao
       write(stdout,'(a,1x,a,i3)') &
         '(pop_set_coupled_forcing)', 'elapsed_months_init_date = ', elapsed_months_init_date+1
     endif
!####### DEBUG

     !*** add NAO_F*wt_nao term to STF below:
     !*** No adjustment for switch-on experiments

!     ADJ_MASK = (c1-LS_MASK)*(c1-IFRAC)   !adjustment mask for this time step 
     NAO_F_NOW = NAO_F*(c1-IFRAC)*wt_nao   !NAO HFLX foring for this time step

     WORK1 = NAO_F_NOW*TAREA
!     WORK2 = TAREA*ADJ_MASK

   ! compute imbalance
     h_imb = global_sum(WORK1, distrb_clinic, field_loc_center, RCALCT)
!     h_adj = h_imb/global_sum(WORK2, distrb_clinic, field_loc_center, RCALCT)

!     ADJ_F = h_adj*ADJ_MASK; 
   else
     NAO_F_NOW = c0
!     ADJ_F = c0
   endif
...
!####### DEBUG
   if (my_task == master_task) then
     write(stdout,'(a,1x,i4.4,a,i2.2,a,i2.2,1x,a,e14.6)') &
       '(pop_set_coupled_forcing)',iyear,'-',imonth,'-',iday, 'h_imb = ',h_imb
!      write(stdout,'(a,1x,i4.4,a,i2.2,a,i2.2,1x,a,F16.6)') &
!        '(pop_set_coupled_forcing)',iyear,'-',imonth,'-',iday, 'h_adj = ',
!        h_adj
     write(stdout,blank_fmt)
   endif
!####### DEBUG
...
!***********************************************************************
!BOP
! !IROUTINE: nao_ramp_weights
! !INTERFACE:

 subroutine nao_ramp_weights (wt_nao)

! !DESCRIPTION:
!  Create a ramping function for use in NAO experiments.  For now, the
!  ramping specifications are hard-coded in this routine; the code
!  could be generalized to work with arbitrary ramp-up and ramp-down
!  endpoints.
!
!  NOTE: There is an underlying assumption in this routine that it is
!        called from pop_set_forcing_coupled, which is called PRIOR to
!        time_manager in subroutine step.  Code modification would be
!        needed if this routine were called AFTER time_manager in step.
!
! !INPUT/OUTPUT PARAMETERS:
!
   real (r8), intent (out) :: wt_nao ! nao ramping weight for this timestep
...
!
!-----------------------------------------------------------------------
   integer (int_kind) :: elap_day_ramp_dn_strt, elap_day_ramp_dn_end
   integer (int_kind) :: elap_day_ramp_up_end, elap_day_ramp_up_strt
   logical (log_kind) :: nao_first = .true.
   real    (r8)       :: ramp_beg_nstep, ramp_end_nstep, ramp_cur_nstep
   save

!-----------------------------------------------------------------------
!  for now, define wt_nao over the course of the year . This is inefficient,
!   but getting the calendar information at run time is much, much easier
!   than pre-computing the weights.  In the future, weights could be
!   computed and stored over the course of the first year, and then
!   in subsequent years, use the stored values rather than recomputing them.
!-----------------------------------------------------------------------

  if (nao_first) then
    call ymd2eday (0, 3,15, elap_day_ramp_dn_strt)
    call ymd2eday (0, 4,15, elap_day_ramp_dn_end)
    call ymd2eday (0,11,15, elap_day_ramp_up_strt)
    call ymd2eday (0,12,15, elap_day_ramp_up_end)
    nao_first = .false.
  endif

  wt_nao = c1

  if (elapsed_days_this_year >= elap_day_ramp_dn_strt .and. &
      elapsed_days_this_year <= elap_day_ramp_dn_end) then
     if (elapsed_days_this_year == elap_day_ramp_dn_strt .and. &
         nsteps_this_day == nsteps_per_day) then
        wt_nao = c1
     else
        !*** ramp down  
        ramp_beg_nstep = elap_day_ramp_dn_strt*nsteps_per_day - nsteps_per_day + 2
        ramp_end_nstep = elap_day_ramp_dn_end*nsteps_per_day+1

        if (nsteps_this_day == nsteps_per_day) then
           ramp_cur_nstep = (elapsed_days_this_year-elap_day_ramp_dn_strt-1)* &
                             nsteps_per_day+nsteps_this_day
        else
           ramp_cur_nstep = (elapsed_days_this_year-elap_day_ramp_dn_strt)* &
                             nsteps_per_day+nsteps_this_day
        endif

        wt_nao = c1 - ramp_cur_nstep/(ramp_end_nstep-ramp_beg_nstep)
     endif
  endif

  if (elapsed_days_this_year >= elap_day_ramp_up_strt .and. &
      elapsed_days_this_year <= elap_day_ramp_up_end) then
     if (elapsed_days_this_year == elap_day_ramp_up_strt .and. &
         nsteps_this_day == nsteps_per_day) then
        wt_nao = c0
     else
        !*** ramp up
        ramp_beg_nstep = elap_day_ramp_up_strt*nsteps_per_day - nsteps_per_day + 2
        ramp_end_nstep = elap_day_ramp_up_end*nsteps_per_day + 1

        if (nsteps_this_day == nsteps_per_day) then
           ramp_cur_nstep = (elapsed_days_this_year-elap_day_ramp_up_strt-1)* &
                             nsteps_per_day+nsteps_this_day
        else
           ramp_cur_nstep = (elapsed_days_this_year-elap_day_ramp_up_strt)* &
                             nsteps_per_day+nsteps_this_day
        endif

        wt_nao = c0 + ramp_cur_nstep/(ramp_end_nstep-ramp_beg_nstep)
     endif
  endif

  if (elapsed_days_this_year > elap_day_ramp_dn_end .and. &
      elapsed_days_this_year < elap_day_ramp_up_strt) then
      wt_nao = c0
  endif
...
!EOC
 ...
 end subroutine nao_ramp_weights
...
!-----------------------------------------------------------------------
   ...
   do iblock = 1,nblocks_clinic
      ...
      this_block = get_block(blocks_clinic(iblock),iblock)
         ...
         call accumulate_tavg_field(NAO_F_NOW(:,:,iblock),  tavg_NAO_F,iblock,1)
   end do
