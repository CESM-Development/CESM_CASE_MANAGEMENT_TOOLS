!|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
 ...
 module forcing_sfwf
! CMB added option for formulation to be hosing, which can be used in 
! coupled mode
! Testing gave bfb with not hosed run, when namelist was set to hose
! but hosing input file had all zeros.
! Important because the namelist variables are fussy and have
! potential interdependency.
! Be sure you do not hose in the marginal seas !!!
...
! I do this BEFORE hosing. So hosing if in the marginal 
! seas will destroy balance. 
...
!-----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
    write(stdout,'(a70)') &
        '   CMB added a new sfwf type called hosing, be sure to use it'
   endif
...
! CMB The namelist is still read after this and can alter these defaults
   ...
   precip_fact_const      = c1 ! !!!IRS, this wasn't in CESM1 - will it mess up pacemaker??
...
!-----------------------------------------------------------------------
   ...
   case ('hosing')
      ...
      allocate(sfwf_data_names(sfwf_data_num_fields), &
               sfwf_bndy_loc  (sfwf_data_num_fields), &
               sfwf_bndy_type (sfwf_data_num_fields))
      sfwf_data_names(sfwf_data_hosing) = 'HOSING'  ! CMB I think this sets up the variable name for nc file
...
! CMB CCSM expects tracer concentration to be zero
   real (r8), dimension(nx_block,ny_block,nt,max_blocks_clinic), &
      intent(inout) :: &
      STF,    &! surface tracer fluxes at current timestep
...
!-----------------------------------------------------------------------
   ...
   do iblock=1,nblocks_clinic
      ...
      SFWF_COMP(:,:,iblock,sfwf_comp_hosing) = &
         SFWF_COMP(:,:,iblock,sfwf_comp_hosing)*salinity_factor
