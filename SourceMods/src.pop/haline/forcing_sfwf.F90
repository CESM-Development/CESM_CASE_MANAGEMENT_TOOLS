...
!
! !REVISION HISTORY:
!  SVN:$Id: forcing_sfwf.F90 85444 2017-06-01 15:43:54Z mlevy@ucar.edu $
!
! !USES:
   ...
   use POP_DistributionMod !FSC debug
   use POP_ErrorMod
   use POP_GridHorzMod
   use POP_FieldMod
   use POP_HaloMod
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      sfwf_hosing_shr_stream_year_first, & ! first year in stream to use
      sfwf_hosing_shr_stream_year_last,  & ! last year in stream to use
      sfwf_hosing_shr_stream_year_align    ! align sfwf_hosing_shr_stream_year_first with this model year

   character(char_len) :: &
      sfwf_hosing_shr_stream_file          ! file containing domain and input data

   type(strdata_input_type) :: &
      sfwf_inputlist                         ! pop stream datatype

   logical (log_kind) :: first_call_strdata_create

   integer (int_kind) :: &
      sfwf_hosing_shr_strdata_advance_timer  ! timer
...
!-----------------------------------------------------------------------
   ...
   namelist /forcing_sfwf_nml/ sfwf_data_type,      sfwf_data_inc,    &
                               ...
                               precip_fact_const,                     &
                               sfwf_hosing_shr_stream_year_first,     &
                               sfwf_hosing_shr_stream_year_last,      &
                               sfwf_hosing_shr_stream_year_align,     &
                               sfwf_hosing_shr_stream_file
...
!-----------------------------------------------------------------------
   ...
   sfwf_hosing_shr_stream_year_first = 1                                                                                              
   sfwf_hosing_shr_stream_year_last  = 1                                                                                              
   sfwf_hosing_shr_stream_year_align = 1                                                                                              
   sfwf_hosing_shr_stream_file       = 'unknown-sfwf_hosing_shr_stream'
   ...
   call broadcast_scalar(sfwf_hosing_shr_stream_year_first, master_task)                                                              
   call broadcast_scalar(sfwf_hosing_shr_stream_year_last , master_task)                                                              
   call broadcast_scalar(sfwf_hosing_shr_stream_year_align, master_task)                                                              
   call broadcast_scalar(sfwf_hosing_shr_stream_file      , master_task)
...
!-----------------------------------------------------------------------
   ...
   case ('hosing')
      !use shr_stream for hosing
      first_call_strdata_create = .true.
...
!-----------------------------------------------------------------------
   ...
   case ('n-hour')
      ...
      write(stdout,*) 'sfwf_data_time: ', sfwf_data_time
      write(stdout,*) 'sfwf_data_inc: ', sfwf_data_inc
      write(stdout,*) 'sfwf_interp_type: ', sfwf_interp_type
      write(stdout,*) 'sfwf_data_next: ', sfwf_data_next
      write(stdout,*) 'sfwf_data_time_min_loc: ', sfwf_data_time_min_loc
      write(stdout,*) 'sfwf_data_update: ', sfwf_data_update
      write(stdout,*) 'sfwf_data_type: ', sfwf_data_type

      do n=1,sfwf_interp_order
         call get_forcing_filename(forcing_filename,  sfwf_filename, &
                                   sfwf_data_time(n), sfwf_data_inc)

         write(stdout,*) 'forcing_filename: ', forcing_filename
         write(stdout,*) 'sfwf_data_time(n): ', sfwf_data_time(n)
         write(stdout,*) 'sfwf_data_inc; ', sfwf_data_inc
         forcing_file = construct_file(sfwf_file_fmt,                 &
                                       full_name=trim(sfwf_filename), &
                                       record_length = rec_type_dbl,  &
   ...
   case ('shr_stream')
      ...
      case ('hosing')
         ...
         allocate( SFWF_DATA(nx_block,ny_block,max_blocks_clinic,1,1))
         SFWF_DATA = c0

         call POP_strdata_type_set(sfwf_inputlist,           &
            file_name   = sfwf_hosing_shr_stream_file,       &
            field       = 'HOSING',                          &
            timer_label = 'sfwf_data',                       &
            year_first  = sfwf_hosing_shr_stream_year_first, &
            year_last   = sfwf_hosing_shr_stream_year_last,  &
            year_align  = sfwf_hosing_shr_stream_year_align)

         call get_timer(sfwf_hosing_shr_strdata_advance_timer, &
                           'SFWF_HOSING_SHR_STRDATA_ADVANCE',1, distrb_clinic%nprocs)
         if (my_task == master_task) then
            write(stdout,blank_fmt)
            write(stdout,*) '  SFWF hosing shr_stream options: '
            write(stdout,*) '   year_first = ', sfwf_hosing_shr_stream_year_first
            write(stdout,*) '   year_last = ', sfwf_hosing_shr_stream_year_last
            write(stdout,*) '   year_align = ', sfwf_hosing_shr_stream_year_align
            write(stdout,*) '   file = ', sfwf_hosing_shr_stream_file
         endif
...
!-----------------------------------------------------------------------
   ...
   integer (POP_i4) :: proc, iLoc, jLoc, locBlock, errc !debut FSC
   integer (POP_i4) :: proc2, iLoc2, jLoc2, locBlock2, errc2 !debut FSC
   ...
   case ('shr_stream')
      ...
      case ('hosing')
         ...
         if (first_call_strdata_create) then
            ...
            call POP_strdata_create(sfwf_inputlist)
         endif
         first_call_strdata_create = .false.

         call timer_start(sfwf_hosing_shr_strdata_advance_timer)
         call POP_strdata_advance(sfwf_inputlist)
         call timer_stop(sfwf_hosing_shr_strdata_advance_timer)

         ! process sfwf hosing forcing
         SFWF_DATA(:,:,:,:,:) = c0
         n = 0
         do iblock = 1, nblocks_clinic
            this_block = get_block(blocks_clinic(iblock),iblock)
            do j=this_block%jb,this_block%je
                do i=this_block%ib,this_block%ie
                  n = n + 1
                  SFWF_DATA(i,j,iblock,1,1) = &
                     sfwf_inputlist%sdat%avs(1)%rAttr(1,n)
                 enddo
             enddo
         enddo

         call POP_HaloUpdate(SFWF_DATA(:,:,:,1,1),POP_haloClinic, &
                             POP_gridHorzLocCenter,          &
                             POP_fieldKindScalar, errorCode, &
                             fillValue = 0.0_POP_r8)
         if (errorCode /= POP_Success) then
            call exit_POP(sigAbort, 'get_sfwf_hosing_data' /&
               &/ ': error updating halo for sfwf fields')
         endif

         ! find block containing global (i,j) = (10, 350)
         call POP_DistributionGetPointLoc(POP_distrbClinic, 10, 350 ,   &
                                          proc, iLoc, jLoc,          &
                                          locBlock, errc)
         ! find block containing global (i,j) = (10, 300)
         call POP_DistributionGetPointLoc(POP_distrbClinic, 10, 300 ,   &
                                          proc2, iLoc2, jLoc2,          &
                                          locBlock2, errc2)
         ...
         do iblock=1,nblocks_clinic
            ...
            STF(:,:,2,iblock) = STF(:,:,2,iblock) + &
                              SFWF_DATA(:,:,iblock,1,1) * &
                              salinity_factor

            if (this_block%block_id == proc) then
                write(stdout,blank_fmt)
                write(stdout,*) 'proc, iLoc, jLoc : ', proc, iLoc, jLoc
                write(stdout,*) 'TLON, TLAT= ', TLON(iLoc,jLoc,iblock)*180./pi, TLAT(iLoc,jLoc,iblock)*180./pi
                write(stdout,*) 'STF(10, 350): ', STF(iLoc,jLoc,2,iblock)
                write(stdout,*) 'SFWF_DATA(10, 350): ', SFWF_DATA(iLoc,jLoc,iblock,1,1)
            endif

            if (this_block%block_id == proc2) then
                write(stdout,blank_fmt)
                write(stdout,*) 'proc2, iLoc2, jLoc2 : ', proc2, iLoc2, jLoc2
                write(stdout,*) 'TLON, TLAT= ', TLON(iLoc2,jLoc2,iblock)*180./pi, TLAT(iLoc2,jLoc2,iblock)*180./pi
                write(stdout,*) 'STF(10, 300): ', STF(iLoc2,jLoc2,2,iblock)
                write(stdout,*) 'SFWF_DATA(10, 300): ', SFWF_DATA(iLoc2,jLoc2,iblock,1,1)
            endif
