...
! !PUBLIC DATA MEMBERS:
   ...
   integer (int_kind), public :: &
      ...
      shf_data_cldfrac,  &
      shf_data_windspd,  &
      shf_comp_heatflux, &  ! CMB for either alyssa_restoring or heating
      shf_comp_qsw,      &
      shf_comp_qlw,      &
      shf_comp_qlat,     &
      shf_comp_qsens,    &
...
!-----------------------------------------------------------------------
   ...
   real (r8) ::          &
      ...
      shf_restore_tau,   &! CMB use this for alyssa_restore, ignoring weak/strong stuff
      shf_restore_rtau,  &
...
!  the following is necessary for stuff CMB added
   integer (int_kind) :: &
      shf_data_flxio
...
!                               STR SST climatology.
   ...
   real (r8), allocatable, dimension(:,:,:)  :: &
      SHF_MASK   ! CMB mask to weight forced shf_comp  
...
!----------------------------------------------------------------------
   ...
   real (r8) :: stf_sum1 ! for global sum diag
...
!-----------------------------------------------------------------------
   ...
   case ('alyssa_restoring')
      lsw_absorb = .true. ! CMB not sure about this
      shf_data_num_fields = 1
      allocate(shf_data_names(shf_data_num_fields), &
               shf_bndy_loc  (shf_data_num_fields), &
               shf_bndy_type (shf_data_num_fields))
   ...
   case ('heating')
      lsw_absorb = .true. ! CMB not sure what this is
      shf_data_num_fields = 1
      allocate(shf_data_names(shf_data_num_fields), &
               shf_bndy_loc  (shf_data_num_fields), &
               shf_bndy_type (shf_data_num_fields))
      shf_data_flxio        = 1
      shf_data_names(shf_data_flxio) = 'APPLIED_SHF'
      shf_bndy_loc  (shf_data_flxio) = field_loc_center
      shf_bndy_type (shf_data_flxio) = field_type_scalar
...
! CMB make mask for our use

      allocate( SHF_MASK(nx_block,ny_block,max_blocks_clinic))
      SHF_MASK = c0
      ...
      io_mask = construct_io_field( &
                      trim('SHF_MASK'),               &
                      dim1=i_dim, dim2=j_dim,         &
                      field_loc = field_loc_center,   &
                      field_type = field_type_scalar, &
                      d2d_array=SHF_MASK)
         call data_set(forcing_file,'define',io_mask)
         call data_set(forcing_file,'read'  ,io_mask)
         call destroy_io_field(io_mask)
         call data_set(forcing_file,'close')
         call destroy_file(forcing_file)

   stf_sum1 = global_sum(SHF_MASK,distrb_clinic,field_loc_center)
   if (my_task == master_task) then
    write(stdout,'(a30,a30)') 'Reading SHF_MASK from file ', trim(shf_filename)
    write(stdout,'(a30,(e12.3))') &
        'SHF_MASK global sum is ', stf_sum1
   endif
     ...
     case ('alyssa_restoring')
      ...
      case ('heating')
         ...
         io_flxio = construct_io_field( &
                      trim(shf_data_names(shf_data_flxio)),       &
                      dim1=i_dim, dim2=j_dim,                        &
                      field_loc = shf_bndy_loc(shf_data_flxio),   &
                      field_type = shf_bndy_type(shf_data_flxio), &
                     d2d_array=SHF_DATA(:,:,:,shf_data_flxio,1))
         call data_set(forcing_file,'define',io_flxio)
         call data_set(forcing_file,'read'  ,io_flxio)
         call destroy_io_field(io_flxio)

    stf_sum1 = global_sum(SHF_DATA(:,:,:,shf_data_flxio,1),distrb_clinic,field_loc_center)
    if (my_task == master_task) then
      write(stdout,'(a30,a30)') 'Reading first SHF_DATA from file ', trim(shf_data_names(shf_data_flxio))
      write(stdout,'(a30,(e12.3))') &
             'SHF_DATA global sum is ', stf_sum1
    endif
...
!-----------------------------------------------------------------------
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      case ('alyssa_restoring', 'partially-coupled')
         io_sst = construct_io_field( &
                      trim(shf_data_names(shf_data_sst)),       &
                      dim1=i_dim, dim2=j_dim, dim3=month_dim,                  &
                      field_loc = shf_bndy_loc(shf_data_sst),   &
      ...
      case ('heating')
         io_flxio = construct_io_field( &
                      ...
                      field_loc = shf_bndy_loc(shf_data_flxio),   &
                      field_type = shf_bndy_type(shf_data_flxio), &
                      d3d_array=TEMP_DATA(:,:,:,:,shf_data_flxio))
         call data_set(forcing_file,'define',io_flxio)
         call data_set(forcing_file,'read'  ,io_flxio)
         call destroy_io_field(io_flxio)
         ...
         do iblock=1,nblocks_clinic
         do n=1,12
            SHF_DATA (:,:,iblock,shf_data_flxio,n) = &
            TEMP_DATA(:,:,n,iblock,shf_data_flxio)
         end do
         end do
         !$OMP END PARALLEL DO
...
!-----------------------------------------------------------------------
   ...
   case ('n-hour')
      ...
      do n = 1, shf_interp_order
        ...
        if (my_task == master_task) then
            write(stdout,blank_fmt)
            write(stdout,'(a23,a)') ' SHF n-hour about to file read: ', &
         ...
         case ('heating')
           io_flxio = construct_io_field( &
                      ...
                      field_loc = shf_bndy_loc(shf_data_flxio),   &
                      field_type = shf_bndy_type(shf_data_flxio), &
                      d2d_array=SHF_DATA(:,:,:,shf_data_flxio,n))

           call data_set(forcing_file,'define',io_flxio)
           call data_set(forcing_file,'read'  ,io_flxio)
           call destroy_io_field(io_flxio)

         case ('restoring','partially-coupled','alyssa_restoring')
      ...
      if (shf_formulation == 'alyssa_restoring' .or. &
          shf_formulation == 'heating'  ) then
         allocate(SHF_COMP(nx_block,ny_block,max_blocks_clinic,  &
                                                 shf_num_comps))
         SHF_COMP = c0   ! initialize
      endif
...
!-----------------------------------------------------------------------
   ...
   case ('annual')
      ...
      case ('alyssa_restoring')
         call calc_shf_alyssa_restoring(1)

      case ('heating')
         call calc_shf_heating(1)
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      case ('alyssa_restoring')
         call calc_shf_alyssa_restoring(12)

      case ('heating')
         call calc_shf_heating(12)
   ...
   case('n-hour')
      ...
      if (thour00 >= shf_data_update) then
         ...
         if (my_task == master_task) then
            write(stdout,blank_fmt)
            write(stdout,'(a23)') ' SHF n-hour update forcing_data'
         endif
         call update_forcing_data(shf_data_time, shf_data_time_min_loc,&
                                  shf_interp_type, shf_data_next,      &
                                  shf_data_update, shf_data_type,      &
      ...
      if (thour00 >= shf_interp_next .or. nsteps_run == 0) then
         if (my_task == master_task) then
            write(stdout,blank_fmt)
            write(stdout,'(a23)') ' SHF n-hour interpolate forcing_data'
         endif
        ...
        if (nsteps_run /= 0) shf_interp_next = &
         ...
         stf_sum1 = global_sum(SHF_DATA(:,:,:,shf_data_flxio,0),distrb_clinic,field_loc_center)
         if (my_task == master_task) then
            write(stdout,blank_fmt)
            write(stdout,'(a30,(e12.3))') &
             'SHF_DATA global sum interp_next ', stf_sum1
         endif
      endif
      ...
      case ('alyssa_restoring')
         call calc_shf_alyssa_restoring(shf_interp_order)

      case ('heating')
         call calc_shf_heating(shf_interp_order)
...
!***********************************************************************
!BOP
! !IROUTINE: calc_shf_heating
! !INTERFACE:

 subroutine calc_shf_heating(time_dim)

! !DESCRIPTION:
!  Calculates some stuff
!  for heating formulation. These components will later be 
...
!-----------------------------------------------------------------------
   ...
   stf_sum0 = global_sum(SHF_DATA(:,:,:,shf_data_flxio,now),distrb_clinic,field_loc_center)
   stf_sum1 = global_sum(SHF_COMP(:,:,:,shf_comp_heatflux),distrb_clinic,field_loc_center)
   if (my_task == master_task) then
    write(stdout,'(a30,(e12.3))') &
        'HEATING SHF_COMP global sums 1 and 2 ', stf_sum1
   endif
   ...
   do iblock=1,nblocks_clinic
      ...
      SHF_COMP(:,:,iblock,shf_comp_heatflux) =    &
                              SHF_MASK(:,:,iblock)*                   &
                              SHF_DATA(:,:,iblock,shf_data_flxio,now)
...
!----------------------------------------------------------------------
      ...
      SHF_COMP(:,:,iblock,shf_comp_heatflux) =               &
             SHF_COMP(:,:,iblock,shf_comp_heatflux)*hflux_factor

   end do ! block loop
   !$OMP END PARALLEL DO

   stf_sum2 = global_sum(SHF_COMP(:,:,:,shf_comp_heatflux),distrb_clinic,field_loc_center)

   if (my_task == master_task) then
    write(stdout,'(a30,3(e12.4))') &
        'HEATFLUX SHF_COMP global sums ', stf_sum0, stf_sum1, stf_sum2
   endif

!----------------------------------------------------------------------
!EOC

 end subroutine calc_shf_heating

!***********************************************************************
!BOP
! !IROUTINE: calc_shf_alyssa_restoring
! !INTERFACE:

 subroutine calc_shf_alyssa_restoring(time_dim)

! !DESCRIPTION:
!  Calculates some stuff
!  for alyssa_restoring formulation. These components will later be
!  added to shf_comp_cpl component in set_coupled_forcing
!  (forcing_coupled) to form the total surface heat flux.
!
!  The only forcing dataset (on t-grid) is
...
!-----------------------------------------------------------------------
   ...
   do iblock=1,nblocks_clinic
      ...
      SHF_COMP(:,:,iblock,shf_comp_heatflux) = shf_restore_rtau*dz(1)* &
                              SHF_MASK(:,:,iblock)*                   &
...
!              SHF_COMP(:,:,iblock,shf_comp_heatflux)*hflux_factor
   ...
   if (my_task == master_task) then
    write(stdout,'(a30,2(e12.3))') &
        'HEATFLUX SHF_COMP global sums 1 and 2 ', stf_sum1, stf_sum2
   endif

!----------------------------------------------------------------------
!EOC

 end subroutine calc_shf_alyssa_restoring
