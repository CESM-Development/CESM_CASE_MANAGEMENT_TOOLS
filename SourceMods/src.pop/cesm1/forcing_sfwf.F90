...
!Aixue Mod
   real (r8), public, allocatable, dimension(:,:,:) :: &
      HOSING_COMP       !Hosing flux read in
!Aixue MOd End
   real (r8), public, allocatable, dimension(:,:,:,:,:) :: &
      TFW_COMP
...
!Aixue Mod
   hosing_filename          = '/glade/p/cgd/ccr/people/ahu/cesm1_hosing/&
				b.e22.B1850LENS.hosingCoast/SourceMods/src.pop2/&
                                hosing_PI_LabCoast2.5Sv.nc'
!Aixue Mod End
...
!Aixue Mod
   call broadcast_scalar(hosing_filename,        master_task)
!Aixue Mode End
!-----------------------------------------------------------------------
!
...
!Aixue Mod
     allocate ( HOSING_COMP(nx_block,ny_block,max_blocks_clinic))
     HOSING_COMP = c0
!!      allocate(sfwf_data_names(1), &
!!               sfwf_bndy_loc  (1), &
!!               sfwf_bndy_type (1))
!!      sfwf_data_names(1) = 'HOS'
!!      sfwf_bndy_loc  (1) = field_loc_center
!!      sfwf_bndy_type (1) = field_type_scalar
      
     forcing_file=construct_file("nc",         &
                                 full_name=trim(hosing_filename),   &
                                 record_length = rec_type_dbl,      &
                                 recl_words=nx_global*ny_global)
     call data_set(forcing_file,"open_read")
     i_dim = construct_io_dim('i',nx_global)
     ...
     io_sss=construct_io_field('HOSING',  &
                               dim1=i_dim,dim2=j_dim, &
                               field_loc = field_loc_center,        &
                               field_type = field_type_scalar,      &
                               d2d_array=HOSING_COMP)
     call data_set(forcing_file,"define",io_sss)
     call data_set(forcing_file,"read",io_sss)
     call data_set(forcing_file,"close")
     call destroy_io_field(io_sss)
...
!Aixue Mod
!   hosing_filename          = '/glade/p/work/ahu/BC5_ne120_t12_pop62_hosingNew/&
!                                SourceMods/src.pop2/hosing_highRes_0.13Sv.nc'
!Aixue Mod End
