...
!-----------------------------------------------------------------------
   ...
   do n = 1,nt
     call define_tavg_field(tavg_DIA_IMPVF_TRACER(n,1), 'DIA_IMPVF_'     /&
  ...
  &/ ' Flux Across Bottom Face from Diabatic Implicit Vertical Mixing', &
     ...
     call define_tavg_field(tavg_DIA_IMPVF_TRACER(n,2), 'DIA_IMPVF_'     /&
...
!-----------------------------------------------------------------------
   ...
   do n = 1,nt
      mt2 = min(n,size(VDC,DIM=4))
      if (accumulate_tavg_now(tavg_DIA_IMPVF_TRACER(n,1))) then
         do k=1,km-1
            ...
            call accumulate_tavg_field(WORK2,tavg_DIA_IMPVF_TRACER(n,1),bid,k)
            call accumulate_tavg_field(WORK2,tavg_DIA_IMPVF_TRACER(n,2),bid,k)
         end do
      endif
   end do
