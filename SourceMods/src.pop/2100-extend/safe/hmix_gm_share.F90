...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_UISOP_2,      &   ! zonal      isopycnal velocity 5-day mean
      tavg_VISOP,        &   ! meridional isopycnal velocity
      tavg_VISOP_2,      &   ! meridional isopycnal velocity 5-day mean
...
!-----------------------------------------------------------------------
   ...
   if ( diag_gm_bolus ) then
     ...
     call define_tavg_field (tavg_UISOP_2, 'UISOP_2', 3,            &
      long_name='Bolus Velocity in grid-x direction (diagnostic)',  &
                   units='cm/s', grid_loc='3211',                   &
                   coordinates='ULONG TLAT z_t time')
     ...
     call define_tavg_field (tavg_VISOP_2, 'VISOP_2', 3,            &
      long_name='Bolus Velocity in grid-y direction (diagnostic)',  &
                   units='cm/s', grid_loc='3121',                   &
                   coordinates='TLONG ULAT z_t time')
...
! !INPUT PARAMETERS:
   ...
   integer (int_kind), dimension(nt,2), intent(in) :: &
...
!-----------------------------------------------------------------------
   ...
   if ( mix_pass /= 1 ) then
        if ( diag_gm_bolus ) then
            ...
            call accumulate_tavg_field (U_ISOP, tavg_UISOP_2, bid, k) 
            call accumulate_tavg_field (V_ISOP, tavg_VISOP, bid, k) 
            call accumulate_tavg_field (V_ISOP, tavg_VISOP_2, bid, k) 
        ...
        do n = 1,nt
          if (accumulate_tavg_now(tavg_HDIFE_TRACER(n,1))) then
            if (partial_bottom_cells) then
               do j=this_block%jb,this_block%je
               do i=this_block%ib,this_block%ie
            ...
            call accumulate_tavg_field(WORK1,tavg_HDIFE_TRACER(n,1),bid,k)
            call accumulate_tavg_field(WORK1,tavg_HDIFE_TRACER(n,2),bid,k)
          endif

          if (accumulate_tavg_now(tavg_HDIFN_TRACER(n,1))) then
            if (partial_bottom_cells) then
               do j=this_block%jb,this_block%je
               do i=this_block%ib,this_block%ie
            ...
            call accumulate_tavg_field(WORK1,tavg_HDIFN_TRACER(n,1),bid,k)
            call accumulate_tavg_field(WORK1,tavg_HDIFN_TRACER(n,2),bid,k)
          endif

          if (accumulate_tavg_now(tavg_HDIFB_TRACER(n,1))) then
            if (partial_bottom_cells) then
               do j=this_block%jb,this_block%je
               do i=this_block%ib,this_block%ie
            ...
            call accumulate_tavg_field(WORK1,tavg_HDIFB_TRACER(n,1),bid,k)
            call accumulate_tavg_field(WORK1,tavg_HDIFB_TRACER(n,2),bid,k)
          endif
        enddo
   endif
