...
!-----------------------------------------------------------------------
   ...
   do n = 1,nt
     ...
     call define_tavg_field(tavg_KPP_SRC(n,1),trim(string),3,     &
     ...
     string2 = trim(tracer_d(n)%short_name) /&
               &/ ' tendency from KPP non local mixing term'
     call define_tavg_field(tavg_KPP_SRC(n,2),trim(string),3,     &
                            long_name=trim(string2),            &
                            units=trim(tracer_d(n)%tend_units), &
                            scale_factor=tracer_d(n)%scale_factor,&
                            grid_loc='3111',                    &
...
!-----------------------------------------------------------------------
   ...
   do n=1,nt
      call accumulate_tavg_field(KPP_SRC(:,:,k,n,bid),tavg_KPP_SRC(n,1),bid,k)
      call accumulate_tavg_field(KPP_SRC(:,:,k,n,bid),tavg_KPP_SRC(n,2),bid,k)
   enddo
