...
!-----------------------------------------------------------------------
   ...
   integer (int_kind), dimension(nt,2) :: &
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_UE_TRACER(1,1),'UET',3,                 &
                          long_name='Flux of Heat in grid-x direction',&
                          units='degC/s', grid_loc='3211',             &
                          coordinates='ULONG TLAT z_t time' )

   call define_tavg_field(tavg_UE_TRACER(1,2),'UET_2',3,               &
                          long_name='Flux of Heat in grid-x direction',&
                          units='degC/s', grid_loc='3211',             &
                          coordinates='ULONG TLAT z_t time' )

   call define_tavg_field(tavg_VN_TRACER(1,1),'VNT',3,                   &
                          long_name='Flux of Heat in grid-y direction',&
                          units='degC/s', grid_loc='3121',             &
                          coordinates='TLONG ULAT z_t time')

   call define_tavg_field(tavg_VN_TRACER(1,2),'VNT_2',3,                   &
                          long_name='Flux of Heat in grid-y direction',&
                          units='degC/s', grid_loc='3121',             &
                          coordinates='TLONG ULAT z_t time')

   call define_tavg_field(tavg_WT_TRACER(1,1),'WTT',3,                   &
                          long_name='Heat Flux Across Top Face',       &
                          units='degC/s', grid_loc='3112',             &
                          coordinates='TLONG TLAT z_w time' )

   call define_tavg_field(tavg_WT_TRACER(1,2),'WTT_2',3,                   &
                          long_name='Heat Flux Across Top Face',       &
                          units='degC/s', grid_loc='3112',             &
                          coordinates='TLONG TLAT z_w time' )

   call define_tavg_field(tavg_UE_TRACER(2,1),'UES',3,                  &
                          long_name='Salt Flux in grid-x direction',   &
                          scale_factor=1000.0_r8,                      &
                          units='gram/kilogram/s', grid_loc='3211',    &
                          coordinates='ULONG TLAT z_t time' )

   call define_tavg_field(tavg_UE_TRACER(2,2),'UES_2',3,               &
                          long_name='Salt Flux in grid-x direction',   &
                          scale_factor=1000.0_r8,                      &
                          units='gram/kilogram/s', grid_loc='3211',    &
                          coordinates='ULONG TLAT z_t time' )

   call define_tavg_field(tavg_VN_TRACER(2,1),'VNS',3,                   &
                          long_name='Salt Flux in grid-y direction',   &
                          scale_factor=1000.0_r8,                      &
                          units='gram/kilogram/s', grid_loc='3121',    &
                          coordinates='TLONG ULAT z_t time')

   call define_tavg_field(tavg_VN_TRACER(2,2),'VNS_2',3,                   &
                          long_name='Salt Flux in grid-y direction',   &
                          scale_factor=1000.0_r8,                      &
                          units='gram/kilogram/s', grid_loc='3121',    &
                          coordinates='TLONG ULAT z_t time')

   call define_tavg_field(tavg_WT_TRACER(2,1),'WTS',3,                   &
                          long_name='Salt Flux Across Top Face',       &
                          scale_factor=1000.0_r8,                      &
                          units='gram/kilogram/s', grid_loc='3112',    &
                          coordinates='TLONG TLAT z_w time' )

   call define_tavg_field(tavg_WT_TRACER(2,2),'WTS_2',3,                   &
                          long_name='Salt Flux Across Top Face',       &
                          scale_factor=1000.0_r8,                      &
                          units='gram/kilogram/s', grid_loc='3112',    &
                          coordinates='TLONG TLAT z_w time' )

   call define_tavg_field(tavg_ADV_TRACER(1,1),'ADVT',2,                     &
   ...
   do n=3,nt
      ...
      call define_tavg_field(tavg_UE_TRACER(n,1),                      &
                             'UE_' /&
      ...
      call define_tavg_field(tavg_UE_TRACER(n,2),                      &
                             'UE_' /&
      ...
      call define_tavg_field(tavg_ADV_TRACER(n,1),                     &
                             'ADV_' /&
   ...
   do n=1,nt
      ...
      call define_tavg_field(tavg_ADV_3D_TRACER(n,1),                    &
                             'ADV_3D_' /&
                                     &/ trim(tracer_d(n)%short_name),3,&
                             long_name=trim(tracer_d(n)%short_name)   /&
                                     &/' Advection Tendency',          &
      ...
      call define_tavg_field(tavg_ADV_3D_TRACER(n,2),                  &
                             'ADV_3D_' /&
                                    &/ trim(tracer_d(n)%short_name)   /&
                                    &/'_2',3, &
                             long_name=trim(tracer_d(n)%short_name)   /&
   ...
   if (mix_pass /= 1 ) then
      ...
      do n=1,nt
         if (tadvect_itype(n) == tadvect_centered) then
            ...
            call accumulate_tavg_field(WORK,tavg_UE_TRACER(n,1),bid,k)
         ...
         call accumulate_tavg_field(WORK,tavg_ADV_TRACER(n,1),bid,k)
         ...
         call accumulate_tavg_field(WORK,tavg_ADV_3D_TRACER(n,1),bid,k)
         call accumulate_tavg_field(WORK,tavg_ADV_3D_TRACER(n,2),bid,k)
