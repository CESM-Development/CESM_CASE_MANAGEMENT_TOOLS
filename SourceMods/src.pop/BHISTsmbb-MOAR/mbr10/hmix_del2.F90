...
!-----------------------------------------------------------------------
   ...
   if (mix_pass /= 1) then
      if (accumulate_tavg_now(tavg_HDIFE_TRACER(n,1))) then
         ...
         enddo
         enddo
         call accumulate_tavg_field(WORK,tavg_HDIFE_TRACER(n,1),bid,k)
         call accumulate_tavg_field(WORK,tavg_HDIFE_TRACER(n,2),bid,k)
      endif
      ...
      if (accumulate_tavg_now(tavg_HDIFN_TRACER(n,1))) then
         ...
         enddo
         enddo
         call accumulate_tavg_field(WORK,tavg_HDIFN_TRACER(n,1),bid,k)
         call accumulate_tavg_field(WORK,tavg_HDIFN_TRACER(n,2),bid,k)
      endif
   endif
