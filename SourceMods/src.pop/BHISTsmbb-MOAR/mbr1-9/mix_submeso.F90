...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_USUBM_2,      &   ! zonal      submeso velocity 5-day mean
      tavg_VSUBM,        &   ! meridional submeso velocity 
      tavg_VSUBM_2,      &   ! meridional submeso velocity 5-day mean
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field (tavg_USUBM_2, 'USUBM_2', 3,              &
    long_name='Submeso velocity in grid-x direction (diagnostic)',  &
                 units='cm/s', grid_loc='3211',                     &
                 coordinates='ULONG TLAT z_t time')
   ...
   call define_tavg_field (tavg_VSUBM_2, 'VSUBM_2', 3,              &
    long_name='Submeso velocity in grid-y direction (diagnostic)',  &
                 units='cm/s', grid_loc='3121',                     &
                 coordinates='TLONG ULAT z_t time')
...
!-----------------------------------------------------------------------
     ...
     if ( mix_pass /= 1 ) then
       ...
       call accumulate_tavg_field (U_SUBM, tavg_USUBM_2, bid, k)
       call accumulate_tavg_field (V_SUBM, tavg_VSUBM, bid, k)
       call accumulate_tavg_field (V_SUBM, tavg_VSUBM_2, bid, k)
