...
!  SVN:$Id$
!
! Modified by Isla Simpson 28th May 2020, to include pacemaker option
! Based on CESM1 SourceMods at /glade/work/nanr/toolsPacemaker/SourceMods/src.pop2
! !USES:
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      tavg_HEAT_F,       &! tavg id for heating flux
...
!-----------------------------------------------------------------------
   ...
   if ( shf_formulation == 'heating' .or. &
        shf_formulation == 'alyssa_restoring' ) then  !CMB
      call define_tavg_field(tavg_HEAT_F,'HEAT_F',2,                           &
                          long_name='Heat Flux Forcing to POP',               &
...
!   if ( lcoupled .and. shf_formulation /= 'partially-coupled' ) then
  if ( lcoupled .and. shf_formulation /= 'partially-coupled' .and. &
      shf_formulation /= 'heating' .and. shf_formulation /= 'alyssa_restoring') then
     shf_num_comps = 1
     shf_comp_qsw  = 1
...
!       shf_data_type /= 'none') then
  if (lcoupled .and. shf_formulation /= 'partially-coupled' .and. &
       shf_formulation /= 'heating' .and.  &
       shf_formulation /= 'alyssa_restoring' .and. &
       shf_data_type /= 'none') then !CMB
       message =   &
...
!-----------------------------------------------------------------------
   ...
   do iblock = 1, nblocks_clinic
      ...
      if ( shf_formulation == 'heating' .or. &
                shf_formulation == 'alyssa_restoring' ) then
        SHF_COMP(:,:,iblock,shf_comp_cpl) = STF(:,:,1,iblock)
      endif
      ...
      if ( sfwf_formulation == 'partially-coupled' ) then
        ...
        if (sfc_layer_type == sfc_layer_varthick .and.  &
          ...
          ! CMB this might work but would still need much add'n coding elsewhere
...
!-----------------------------------------------------------------------
  ...
  real (r8) stf_sum2
   ...
   if ( shf_formulation == 'partially-coupled' ) then
     ...
     enddo
     !$OMP END PARALLEL DO
   else if ( shf_formulation == 'heating'  .or. &
             shf_formulation == 'alyssa_restoring' ) then
     !$OMP PARALLEL DO PRIVATE(iblock)
     do iblock=1,nblocks_clinic
       STF(:,:,1,iblock) =  SHF_COMP(:,:,iblock,shf_comp_heatflux)     &
                   + SHF_COMP(:,:,iblock,shf_comp_cpl)
!       STF(:,:,1,iblock) =  SHF_COMP(:,:,iblock,shf_comp_cpl) ! for testing
     enddo
     !$OMP END PARALLEL DO
   endif
...
! even if you set lms_balance to be T
   ...
   if (my_task == master_task) then
    write(stdout,'(a70)') &
        'STF is combined coupler and hosing forcing and/or heating flux'
...
!-----------------------------------------------------------------------
   ...
   do iblock = 1,nblocks_clinic
      ...
      this_block = get_block(blocks_clinic(iblock),iblock)
         ...
         if ( shf_formulation == 'heating' .or. &
              shf_formulation == 'alyssa_restoring' ) then  !CMB
           WORK = SHF_COMP(:,:,iblock,shf_comp_heatflux)/hflux_factor
           call accumulate_tavg_field(WORK, tavg_HEAT_F,iblock,1)
         endif
