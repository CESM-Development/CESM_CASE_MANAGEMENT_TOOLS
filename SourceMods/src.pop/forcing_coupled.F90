...
!-----------------------------------------------------------------------
 ...
 !**************************************************************
 !slarson start: here we will be reading in climatological
 ! taux and tauy files, and overwriting what the ocean 
 ! model is to receive from the coupler. The forcing files are
 ! obtained from a CTRL version of B1850 and are directly
 ! outputted by the coupler in the coupler history file. 
 ! climatology is 6-hourly. 
 !**************************************************************

   integer, parameter :: dbl_kind = selected_real_kind(13)

   real (kind=dbl_kind) :: taux_g(nx_global,ny_global) !each i-step global var
   real (kind=dbl_kind) :: tauy_g(nx_global,ny_global) !each i-step global var

   integer (kind=int_kind) ::  & !input variable sizes
           varid_u  , & !variable id
           varid_v  , & !variable id
           varid_o  , & !variable id OUTPUT FILE TEST
           fid_u    , & !file id
           fid_v    , & !file id
           fid_o    , & !file id OUTPUT FILE TEST
           ...
           LatDimID , &
           LonDimID , &
           forcingO      !time step to grab from forcing dataset

   character (len=16) :: varname_u = 'x2oavg_Foxx_taux' !field name in netcdf file
   character (len=16) :: varname_v = 'x2oavg_Foxx_tauy' !field name in netcdf file

   integer, dimension(nf90_max_var_dims) :: dimIDs

   !define counter: only want to read in file once/job submission
   integer (kind=int_kind),save :: toread 
   data toread /0/

   real (r8), dimension(nx_block,ny_block,max_blocks_clinic) ::   &
      WORK1_f, WORK2_f        ! contains taux and tauy forcings
   ...
   !define variables if needed
    ...
    tauy_g = c0
      ...
      if (my_task == master_task) then
        ...
        !IMPORTANT NOTE: the ocean model skips the first hour of computation
        !So the first ihour upon initialization is actually 1. BUT the first
        !hour of the day is really 0. Hours 1-24 are counted as 0-23 via ihour

          if ( (ihour .ge. 0) .and. (ihour .le. 5) ) then
           forcingO = (24*(iday_of_year-1) + 6)/6

          else if ( (ihour .ge. 6) .and. (ihour .le. 11) ) then
           forcingO = (24*(iday_of_year-1) + 12)/6

          else if ( (ihour .ge. 12) .and. (ihour .le. 17) ) then
           forcingO = (24*(iday_of_year-1) + 18)/6

          else if ( (ihour .ge. 18) .and. (ihour .le. 23) ) then 
           forcingO = (24*(iday_of_year-1) + 24)/6
 
          end if !end conditionals
          
        write(unit=44,fmt=*)'iday_of_year = ',iday_of_year
        write(unit=44,fmt=*)'ihour = ',ihour
        write(unit=44,fmt=*)'forcingO = ',forcingO
        write(unit=44,fmt=*)'-------------------------------'

        !------------------------------
        ! taux: variable from coupler history file
        ! x2oavg_Foxx_taux: in N/m2; correct sign, POP grid
        !------------------------------
           !get file id; test showed dim(1)=lon; dim(2)=lat; dim(3)=time
           status =nf90_open('x2oavg_Foxx_taux_6hourly.nc',nf90_nowrite, fid_u)
           if (status /= nf90_noerr) then
             write(unit=22,fmt=*)'u forcing file: Cannot open the file'
           end if

           !get variable id
           status = nf90_inq_varid(fid_u,trim(varname_u),varid_u)
           if (status /= nf90_noerr) then 
             write(unit=22,fmt=*)'u forcing file: Cannot find variable taux'
           end if

          !figure out dimension order of input variable
          !Note: this worked well
           status = nf90_inquire_variable(fid_u,varid_u,dimids = dimIDs)
           status = nf90_inquire_dimension(fid_u,dimIDs(1),len =numLons)
           status = nf90_inquire_dimension(fid_u,dimIDs(2),len =numLats)
           status = nf90_inquire_dimension(fid_u,dimIDs(3),len =numTims)
            write(unit=11,fmt=*)'dimIDs = ',dimIDs
            write(unit=11,fmt=*)'assuming dimID(1) numLons = ',numLons
            write(unit=11,fmt=*)'assuming dimID(2) numLats = ',numLats
            write(unit=11,fmt=*)'assuming dimID(3) numTims = ',numTims

           !read in variable
           !! START AND COUNT
           status = nf90_get_var(fid_u,varid_u,taux_g, &
                    start = (/1, 1, forcingO/), &
                    count = (/numLons,numLats,1/))
           if (status /= nf90_noerr) then
             write(unit=22,fmt=*)'u forcing file: cannot get variable taux'
           end if

           status = nf90_close(fid_u)

        !------------------------------
        ! tauy: variable from coupler history file
        ! x2oavg_Foxx_tauy: in N/m2; correct sign, POP grid
        !------------------------------
           !get file id; test showed dim(1)=lon; dim(2)=lat; dim(3)=time
           status =nf90_open('x2oavg_Foxx_tauy_6hourly.nc',nf90_nowrite, fid_v)
           if (status /= nf90_noerr) then
             write(unit=22,fmt=*)'v forcing file: Cannot open the file'
           end if

           !get variable id
           status = nf90_inq_varid(fid_v,trim(varname_v),varid_v)
           if (status /= nf90_noerr) then 
             write(unit=22,fmt=*)'v forcing file: Cannot find variable taux'
           end if

           !read in variable
           !! START AND COUNT
           status = nf90_get_var(fid_v,varid_v,tauy_g, &
                    start = (/1, 1, forcingO/), &
                    count = (/numLons,numLats,1/))
           if (status /= nf90_noerr) then
             write(unit=22,fmt=*)'v forcing file: cannot get variable taux'
           end if

          write(unit=99,fmt=*)'ihour',ihour 
          write(unit=99,fmt=*)'iday_of_year',iday_of_year 
          write(unit=99,fmt=*)'imonth',imonth 

           status = nf90_close(fid_v)

           !<><><><><><><><><><><><><><><><><><>
           ! BEGIN TEST IF READ WORKED
           !<><><><><><><><><><><><><><><><><><>
         if (toread == 0) then !only write test file once per job submission          


           ! open file to write out one timestep of variable to check work
           status = nf90_create(path="testout_ocn.nc",cmode=nf90_clobber, &
                    ncid=fid_o)
           if (status /= nf90_noerr) then
             write(unit=33,fmt=*)'output file: cannot create file testout_ocn.nc' 
           end if

           ! define dimensions
           status = nf90_def_dim(fid_o,"lon",nx_global,LonDimID)
           status = nf90_def_dim(fid_o,"lat",ny_global,LatDimID)
           if (status /= nf90_noerr) then 
             write(unit=33,fmt=*)'output file: Cannot define dimensions' 
           end if

           !define variable
           status = nf90_def_var(fid_o,"tauy_test", nf90_double, &
                    (/LonDimID,LatDimID/),varid_o)
           if (status /= nf90_noerr) then
             write(unit=33,fmt=*)'output file: cannot define variable'
           end if
 
           !***end the define process
           status = nf90_enddef(fid_o)
           if (status /= nf90_noerr) then
             write(unit=33,fmt=*)'output file: cannot end defining variable' 
           end if

           !write out variable
           status = nf90_put_var(fid_o,varid_o,tauy_g)
           if (status /= nf90_noerr) then
             write(unit=33,fmt=*)'output file: cannot write variable'
           end if

           !close file
           status = nf90_close(fid_o)
       ...
       call scatter_global(WORK1_f,taux_g,master_task,distrb_clinic, &
            field_loc_NEcorner,field_type_scalar)
       call scatter_global(WORK2_f,tauy_g,master_task,distrb_clinic, &
            field_loc_NEcorner,field_type_scalar)
...
!-----------------------------------------------------------------------
   ...
   SMFT(:,:,1,:) = (WORK1_f(:,:,:)*cos(ANGLET(:,:,:)) +  & !WORK1 to WORK1_f
                    WORK2_f(:,:,:)*sin(ANGLET(:,:,:)))*  & !WORK2 to WORK2_f
                    RCALCT(:,:,:)*momentum_factor
   SMFT(:,:,2,:) = (WORK2_f(:,:,:)*cos(ANGLET(:,:,:)) -  & !WORK2 to WORK2_f
                    WORK1_f(:,:,:)*sin(ANGLET(:,:,:)))*  & !WORK1 to WORK1_f. END OF SLARSON MODS
                    RCALCT(:,:,:)*momentum_factor
