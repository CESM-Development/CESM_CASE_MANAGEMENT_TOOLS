...
CASE_NAME="v2.LR.SSP370_AUFIRE-cntl.${useyear}-${usemonth}.${mbr}"
CASE_GROUP="v2.LR"

#if [[ ${imbr} -eq "1" ]]
#then
  MAIN_CASE_NAME="v2.LR.SSP370_AUFIRE-cntl.${useyear}-${usemonth}.001"
#fi
CASE_GROUP="v2.LR"
...
if [ "${run}" != "production" ]; then
  ...
  WALLTIME="36:00:00"
  STOP_OPTION="nmonths"
  STOP_N="60" # How often to stop the model, should be a multiple of REST_N
  STOP_DATE="20250101"    # -999 or specify stop date as yyyyddmm without leading zeros
  REST_OPTION="nmonths"
  REST_N="60" # How often to write a restart file
