...
if [[ ${imbr} -eq "1" ]]
then
  MAIN_CASE_NAME="v2.LR.SSP370_AUFIRE-cntl.${useyear}-${usemonth}.${mbr}"
fi
CASE_GROUP="v2.LR"
...
if [ "${run}" != "production" ]; then
  ...
  WALLTIME="24:00:00"
  STOP_OPTION="nmonths"
  STOP_N="48" # How often to stop the model, should be a multiple of REST_N
  STOP_DATE="20230801"    # -999 or specify stop date as yyyyddmm without leading zeros
  REST_OPTION="nmonths"
