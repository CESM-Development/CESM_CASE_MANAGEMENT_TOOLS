...
#array=( 0121 0151 0191 0211 0251 )
#syr=( 1976 1975 1976 1976 1975 )
array=( 0121 )
syr=( 1976 )
for iyr in "${array[@]}"
do
...
CASE_NAME="v2.LR.historical_xtraOutput_${iyr}"
CASE_GROUP="v2.LR"
...
START_DATE="$doyear-01-01"

# Additional options for 'branch' and 'hybrid'
GET_REFCASE=TRUE
RUN_REFDIR="/pscratch/sd/n/nanr/archive/v2.LR.historical_${iyr}/archive/rest/$doyear-01-01-00000/"
RUN_REFCASE="v2.LR.historical_${iyr}"
RUN_REFDATE="${doyear}-01-01"   # same as MODEL_START_DATE for 'branch', can be different for 'hybrid'
...
CASE_ROOT="/pscratch/sd/n/${USER}/E3SMv2-Rerun/${CASE_NAME}/"

# Sub-directories
#CASE_BUILD_DIR=${CASE_ROOT}/build
CASE_BUILD_DIR="/pscratch/sd/n/nanr/E3SMv2-Rerun/exeroot/build"
...
if [ "${run}" != "production" ]; then
  ...
  WALLTIME="12:00:00"
  STOP_OPTION="nmonths"
  STOP_N="24" # How often to stop the model, should be a multiple of REST_N
  STOP_DATE="20150101"    # -999 or specify stop date as yyyyddmm without leading zeros
  REST_OPTION="nmonths"
  REST_N="24" # How often to write a restart file
...
OLD_EXECUTABLE="/pscratch/sd/n/nanr/E3SMv2-Rerun/exeroot/build"
...
cat << EOF >> user_nl_eam
 ...
 avgflag_pertape = 'A','A','I','A','I','A','I'
 ! monthly (h0) A
 ! daily (h1) A
 fincl2 = 'CAPE','CLDTOT','FLDSC','FLNR','FLNS','FLNT','FLUT','FSNR','FSNS','FSNT','LHFLX','SWCF','LWCF','SHFLX','PBLH','PHIS','PRECC','PRECL','PRECZ','PS','SST','TAUBLJX','TAUBLJY','TAUGWX','TAUGWY','TAUX','TAUY','TMQ','PCONVT','TROP_P','TROP_T','TROP_Z','PRECT','FLNSC','FLNTC','FLUTC','FSDSC','FSNSC','FSNTC','FSNTOAC','U','V','OMEGA','Q','T','Z3','ZMDT','ZMDQ','DTCOND','DCQ','CLDICE','CLDLIQ','CLOUD','RELHUM'
 ! 6hourly (h2) I
 fincl3 = 'IVT','UIVT','VIVT','TMQ','U850','V850','Z200','Z500','U200','V200'
 ! 6hourly (h3) A
 ! 3hourly (h4) I
 fincl5 = 'PRECC','PRECL','PRECZ'
 ! monthly (h6) I
...
case_setup() {
    ...
    #./xmlchange EXEROOT="/pscratch/sd/n/nanr/E3SMv2-Rerun/exeroot/build"
    ...
    ics="/pscratch/sd/n/nanr/archive/v2.LR.historical_${iyr}/archive/rest/${doyear}-01-01-00000/"
    ...
    cp ${ics}/rpointer.* ${CASE_RUN_DIR}/
    ln -s ${ics}/v2.LR* ${CASE_RUN_DIR}/

    # fix the modules
    cp /global/u2/n/nanr/CESM_tools/e3sm/v2/scripts/v2.LR.LE-rerun/env_mach/env_mach_specific.xml ${CASE_SCRIPTS_DIR}/
    ./case.setup 
...
runtime_options() {
    ...
    # Segment length
    ./xmlchange STOP_OPTION=${STOP_OPTION,,},STOP_N=${STOP_N},STOP_DATE=${STOP_DATE}
