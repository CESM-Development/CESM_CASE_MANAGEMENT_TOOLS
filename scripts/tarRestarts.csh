...
setenv RESTDIR  /glade/campaign/cgd/ccr/nanr/decadalPrediction/restarts/
...
while ($sr <= $er) 
  ...
  while ($mon <= 12) 
     if ($mon < 10) then
        tar -cvf $RESTDIR/$CASE.0${sr}-0$mon-01-00000.tar $ARCHDIR/$CASE/rest/0${sr}-0$mon-01-00000/
     else
        tar -cvf $RESTDIR/$CASE.0${sr}-$mon-01-00000.tar $ARCHDIR/$CASE/rest/0${sr}-$mon-01-00000/
     endif
     @ mon += 1
  end
@ sr += 1
