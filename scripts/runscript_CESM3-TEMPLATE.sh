...
srcdir='/glade/work/nanr/cesm_tags/CASE_tools/cesm3/scripts/'
compset='B1850C_LTso'
usetag='e30_beta06'
...
caseroot='/glade/derecho/scratch/nanr/workflow-testcase/'
...
RUNTYPE="hybrid"
GETREFCASE=TRUE
RUNREFDIR="cesm2_init"
USEPROJECT="P93300313"
STOP_N=10
STOP_OPTION=nyears
RESUBMIT=10

for mbr in $(seq -f "%03g" 999 999)
do
...
rundir=/glade/derecho/scratch/nanr/workflow-testrun/$casename/run/
...
## Create new case 
#./create_newcase --compset $compset --res $resoln --case $casedir --run-unsupported --workflow
...
./xmlchange RUN_REFDIR=${RUNREFDIR}
./xmlchange GET_REFCASE=${GETREFCASE}
 
./xmlchange JOB_PRIORITY=regular
./xmlchange PROJECT=${USEPROJECT},RESUBMIT=${RESUBMIT},STOP_N=${STOP_N},STOP_OPTION=${STOP_OPTION}
...
# Save script in run directory:
#copy_script() {
    echo $'\n----- Saving run script for provenance -----\n'

    script_provenance_dir=${casedir}/run_script_provenance
    mkdir -p ${script_provenance_dir}
    this_script_name=`basename $0`
    script_provenance_name=${this_script_name}.`date +%Y%m%d-%H%M%S`
    cp -vp ${srcdir}${this_script_name} ${script_provenance_dir}/${script_provenance_name}
#}
