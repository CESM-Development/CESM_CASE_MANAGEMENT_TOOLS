...
set tools = /glade/work/nanr/cesm_tags/CASE_tools/cesm1-heatX/
set myhome = /glade/work/hteng/hteng/soil/ 
...
cd /glade/work/nanr/cesm_tags/cesm1_1_2_LENS_n21/scripts
...
./xmlchange -file env_run.xml -id RUN_STARTDATE -val ${icyr}'-01-01'
./xmlchange -file env_run.xml -id RUN_REFDATE -val ${icyr}'-01-01' 
#./xmlchange -file env_run.xml -id RUN_REFCASE -val 'CAM5.ctrl-'$member 
./xmlchange -file env_run.xml -id RUN_REFCASE -val 'f.e11.F1850C5CN.f09_f09.001'
...
./xmlchange -file env_mach_pes.xml -id NTASKS_CPL -val '576'
...
./xmlchange -file env_mach_pes.xml -id NTASKS_ROF -val '576'
...
#restart
mkdir /glade/scratch/nanr/$casename/bld
mkdir /glade/scratch/nanr/$casename/run
...
if ( $member == 01 ) then
  ...
  mkdir /glade/scratch/nanr/$casename/bld
  cd /glade/scratch/nanr/$casename/bld
  ln  -s /glade/scratch/nanr/CAM5.ctrl-01/bld/cesm.exe  .
endif
...
# copy namelist
#cp {$tools}/namelist/user_nl_clm_zero_yellowstone   ${caseroot}/user_nl_clm 
cp {$tools}/namelist/nanRerun/user_nl_clm-rerunCNTL ${caseroot}/user_nl_clm
cp {$tools}/namelist/nanRerun/user_nl_cam-rerunCNTL ${caseroot}/user_nl_cam
cp {$tools}/namelist/nanRerun/user_nl_cice-rerunCNTL ${caseroot}/user_nl_cice
#cp {$tools}/namelist/user_nl_cice $caseroot
rm -f {$caseroot}/CaseDocs/docn.streams.txt.prescribed
chmod 664 {$caseroot}/Buildconf/docnconf/docn.streams.txt.prescribed  
cp $tools/namelist/user_docn.streams.txt.prescribed $caseroot 

cat $casename.run |  sed 's/01:50:00/02:30:00/g' >& test.run
