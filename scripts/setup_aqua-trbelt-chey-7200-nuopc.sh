...
if ($mbr < 10) then
	setenv CASE q.e21.${COMPSET}.${RESOLN}.${EXPERIMENT}-${TAG}-${NTASKS}-nuopc.00${mbr}
else
	setenv CASE q.e21.${COMPSET}.${RESOLN}.${EXPERIMENT}-${TAG}-${NTASKS}-nuopc.0${mbr}
endif
...
$CESMROOT/cime/scripts/create_newcase --case $CASEROOT --res $RESOLN  --mach $MACHINE --compset $COMPSET --pecount L --run-unsupported --driver nuopc
...
./xmlchange ICE_DOMAIN_PATH=$ICEOCNDOMAIN
...
cp $TOOLSROOT/user_nl_files/user_nl_docn $CASEROOT/
cp $TOOLSROOT/user_nl_files/user_nl_cpl $CASEROOT/
cp $TOOLSROOT/user_nl_files/user_nl_cam-chey $CASEROOT/user_nl_cam
